  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-14">
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-semibold">Downloads</h2><div className="mt-4 space-y-3">{hasActiveLicense ? ((releases || []).length === 0 ? <p className="text-sm text-slate-600">No software release is currently available.</p> : entitledReleases.map((r) => <div key={r.id} className="flex items-center justify-between rounded-xl bg-slate-50 p-4"><div><strong>MarineStruc {r.version}</strong><p className="text-xs text-slate-500">{r.release_notes || "Current release"}</p></div><a href={`/api/download/${r.id}`} className="rounded-lg bg-teal-700 px-3 py-2 text-sm font-semibold text-white">Download</a></div>)) : <p className="text-sm text-slate-600">An active license is required to access private downloads.</p>}{hasActiveLicense && (releases || []).length > 0 && entitledReleases.length === 0 && <p className="text-sm text-slate-600">Your license remains valid, but the current releases are outside your included update entitlement.</p>}</div></div>
